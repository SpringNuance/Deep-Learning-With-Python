"""Test Cases Generator for the notebook "SGD"

 This script allows to the instructors to generate data sets for the notebook "SGD".
...
from reference_solutions import gradient_step_one_feature    # reference_solutions contains implemented funcs
from reference_solutions import gradient_step
from test_case_input import TestCaseInput    # to generate html objects

# jsonpickle includes a built-in numpy extension. 
# If would like to encode sklearn models, numpy arrays, 
# and other numpy-based data then you must enable the numpy extension by registering its handlers:
...
@click.option('--initial-input-size', '-m', default=2, help='Initial size for the input dataset. The consequent datasets will be a multiple of this value', show_default=True)
...
def gradient_step_one_feature_test_generator(cases_count, initial_input_size, lrates, weights):
    ...
    $ python3 generate_test_cases.py gradient_step_one_feature
...
@cli.command(name='gradient_step')
...
def gradient_step_test_generator(cases_count, initial_input_size, lrates, weights):
    ...
    $ python3 generate_test_cases.py gradient_step
